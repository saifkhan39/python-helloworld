...
single_url = vault.get_secret('caiso-oasis-url')+'/oasisapi/SingleZip?'
group_url = vault.get_secret('caiso-oasis-url')+'/oasisapi/GroupZip?'
...
def fnm_mapping(fnm_version: str, area = 'CISO', map_type = 'GEN_RES'):
    ...
    ###: encoding issue req formatting into str before query
    parameters_str = "&".join("%s=%s" % (k,v) for k,v in parameters.items())
    query = requests.get(vault.get_secret('caiso-oasis-url')+'/oasisapi/GroupZip?', params=parameters_str)
