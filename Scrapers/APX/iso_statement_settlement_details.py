...
sql = "SELECT * FROM [apx_caiso].[dbo].[iso_statement_settlement_details]"
...
payload = get_payload(start, trade_date, 'ISO Statement')
...
if not data.empty:
    data.to_sql('iso_statement_settlement_details', con=dbConnect(
        'apx_caiso'), index=False, method=None, if_exists='append')
    print(
        f'ISO Statement Settlement Data from {pd.to_datetime(data.trade_date.dt.date.unique())} Added to ISO Statement Settlement Details Table')
else:
    print(f'No Unique ISO Statement Settlement Data Data to Add from {start} to {trade_date}')
