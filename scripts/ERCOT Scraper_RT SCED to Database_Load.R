...
currentDates <- dbGetQuery(ErcotMarketData,
                           paste0('SELECT distinct [SCED Time Stamp] FROM [dbo].[','60d_RT_Load_Data',']'))
...
ERCOTtoDT <- function(csvName,sqlTable){
  for(i in 1:nrow(table)){
      tryCatch({
        ...
        # cols <- names(upload)[7:ncol(upload)]
        # upload[,(cols) := round(.SD,2), .SDcols=cols]
        blocks <- ceiling(nrow(upload)/10000)
        if(blocks > 0){
          for(i in 1:blocks){
            start <- (i-1)*10000
            end <- ifelse((i*10000)<nrow(upload),i*10000,nrow(upload))
            tryCatch({dbAppendTable(ErcotMarketData,sqlTable, upload[start:end,], row.names=NULL, field.types = NULL)
...
ERCOTtoDT('60d_Load_Resource_Data','60d_RT_Load_Data')
