...
try:
    ...
    load = ut.download_files(file_list=files, header_name = 'LTOTQSE', start_date = load_latest.strftime('%Y-%m-%d'))
    if load.empty == True:
...
try:
    ...
    lrs = ut.download_files(file_list=files, header_name = 'LRSLSE', start_date = lrs_latest.strftime('%Y-%m-%d'))
    if lrs.empty == True:
        print('Still nothing to upload')
    else:
